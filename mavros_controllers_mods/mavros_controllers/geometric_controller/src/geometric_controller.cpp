...
geometricCtrl::geometricCtrl(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
  ...
  mavtwistSub_ = nh_.subscribe("mavros/local_position/velocity_local", 1, &geometricCtrl::mavtwistCallback, this,
                               ros::TransportHints().tcpNoDelay());
  extTrajBoolSub_ = nh_.subscribe("ext_traj/bool", 1, &geometricCtrl::extTrajBoolCallback, this, ros::TransportHints().tcpNoDelay());
  ...
  angularVelPub_ = nh_.advertise<mavros_msgs::AttitudeTarget>("command/bodyrate_command", 1); // remapped to /mavros/setpoint_raw/attitude
...
void geometricCtrl::extTrajBoolCallback(const std_msgs::Bool &msg) {
  extTrajBool_ = msg.data;
}
...
void geometricCtrl::statusloopCallback(const ros::TimerEvent &event) {
  if (sim_enable_) {
    ...
    arm_cmd_.request.value = true;
      // COMBAK: comment these lines out
